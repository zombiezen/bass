...
func Exec(conn *sqlite.Conn, fsys FS, filename string, opts *ExecOptions) error {
	...
	if err != nil {
		return fmt.Errorf("exec %s: %w", filename, stripErrorQuery(err))
	}
	err = exec(stmt, opts)
	resetErr := stmt.Reset()
	if err != nil {
...
func PrepareTransient(conn *sqlite.Conn, fsys FS, filename string) (*sqlite.Stmt, error) {
	...
	stmt, _, err := conn.PrepareTransient(strings.TrimSpace(query))
	if err != nil {
		return nil, fmt.Errorf("prepare %s: %w", filename, stripErrorQuery(err))
	}
	return stmt, nil
}
...
func stripErrorQuery(err error) error {
	// Not using errors.As, because we only want to examine the top of the chain.
